...
export function IssueLogsSection({
	issues,
	className,
}: IssueLogsSectionProps): React.JSX.Element {
	return (
		<section
			className={cn(
				"h-full rounded-xl border border-white/15 bg-white/5 p-4",
				className,
			)}>
				...
				<HoverCard openDelay={120} closeDelay={80}>
					...
					<HoverCardContent
						...
						<p className="mt-1 text-[11px] leading-relaxed text-zinc-300">
							This section lists the latest warnings and critical findings from
							monitoring checks. Each log helps you understand what changed and
							which process needs attention first.
						</p>
					</HoverCardContent>
				</HoverCard>
			</div>
			...
			<div className="mt-3 space-y-2.5">
				{issues.length === 0 ?
					<p className="text-sm text-zinc-300">
						No active warnings in the current cycle.
					</p>
				:	issues.map((issue) => (
						<div
							key={issue.id}
							id={`issue-row-${issue.id}`}
							className="flex scroll-mt-24 items-center gap-2 rounded-lg border border-white/10 p-2.5 transition">
							<span
								className={`h-2.5 w-2.5 rounded-full ${severityDotClass(issue.severity)}`}
							/>
